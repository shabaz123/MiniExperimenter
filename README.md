...
* If you're using a board that can connect to Microsoft IoT Central, then data can be captured by the calculator and locally processed, and then sent back to the microcontroller board for onward forwarding via a protocol called MQTT. This functionality has been tested with ESP32 microcontroller boards. To use it, create a program on the calculator such as the example shown here:
